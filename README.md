...
- **MCP server verified end-to-end against a real MCP client** (`fastmcp.Client` spawning `fact-checker-mcp` as a subprocess over stdio — the identical mechanism Claude Desktop uses). Not yet separately confirmed inside the Claude Desktop app UI itself, but the protocol round-trip (tool discovery, argument passing, structured JSON results) is proven correct.
